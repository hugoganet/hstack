---
id: ADR-0003-kernel-fit-closed-loop
type: adr
status: proposed
owner: hugoganet
...
The triggering observation. An engineer noticed during change planning that the `internal-tooling: true` flag conflates two semantically distinct cases: (A) true internal tooling that never ships on a user path (engineering scripts, CI tooling, dev dashboards), and (B) production code that ships to users but whose user-observable behavior is deferred to a downstream consumer change-spec (foundational prerequisites, plumbing changes). Both legitimately carry "no driving user story for this specific change," and both legitimately set `internal-tooling: true` today, but the kernel's flag treats them identically. The audit-cost difference is real: Category B has user value that lives elsewhere in the change graph, and an auditor reconstructing user-value flow cannot follow that link if the kernel never recorded it. The user explicitly asked for a *general* mechanism for detecting gaps like this rather than a one-off flag-splitting fix.
...
**Option A — Per-flag fix for `internal-tooling`.** Split the flag into `internal-tooling | foundational-prerequisite`, add a validator rule that requires `enables: [<downstream-change-spec-id>]` when `foundational-prerequisite: true`, and ship a small kernel-section addition naming the distinction. **Rejected** because it solves one case and the user explicitly asked for a general mechanism. The flag-conflation pattern would recur — every controlled-vocabulary field in the kernel is at risk of being two-cases-in-one — and the per-case fix doesn't generalize. Worse, a per-flag fix establishes precedent for ad-hoc kernel evolution that ADR-0001 deliberately argued against.
...
**Option C — Five-layer closed-loop subsystem.** **Adopted.** The structure scales beyond the `internal-tooling` case; the human-gated promotion respects the user's explicit constraint; the substrate reuses existing telemetry infrastructure; every component follows a pattern that already exists in the kernel. The cost is real (new machinery to maintain) and acknowledged in the Consequences. The benefit is that hstack becomes self-aware in a bounded, auditable, human-supervised way.
