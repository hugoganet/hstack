...
- **≈ 4.2k tokens off every session and every subagent invocation** (13,709 → 9,472 at 4.83 b/tok). Unlike ADR-0012's saving, which is paid only by sessions that invoke Skills, this one is unconditional and multiplied: a change that spawns five subagents pays the kernel six times, so the per-change saving is ≈ 25k, not ≈ 4.2k. It is the only one of the four context-engineering reductions that compounds with subagent count.
...
- Word counts in the PR body are the measurement of record. The brief's target was ~4,500 words; the arbitrations produced 6,370. The gap is almost exactly the two blocks this ADR declines to touch — § Test immutability at 671 words and the sixteen reconciled session-start load lists at ~590 — plus roughly 600 words of further squeeze that would have to come out of § Mechanical operations and § Stop conditions, both of which are already at the density where the next edit starts deleting carve-outs. The number is reported as found rather than met.
...
**Option D — Cut to ~4,500 words by also thinning § Test immutability and the reconciled session-start lists.** This is the arithmetic the brief's target implies, and it is the only way to reach it: the two blocks are 1,261 words between them, and the remaining ~600 would come out of § Mechanical operations' carve-outs. Rejected on both counts. The load lists were arbitrated one commit ago and re-opening them makes ADR-0012's ledger a formality; test immutability is the rule with no validator id, no CI check, and a failure that is silent and ships. The PR lands at 6,370 and says so, rather than hitting the number on the two sections least able to afford it. If 4,500 is still wanted, the honest route is Option B — split the kernel — not a deeper cut here.
