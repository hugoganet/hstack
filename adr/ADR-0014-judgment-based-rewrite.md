...
- **The five deleted counter-mechanisms are net prose reduction on agent-loaded bodies.** `adversarial-reviewer.md` and the two tech-debt Skills lose more than the goal statements add, in files that are loaded on invocation.
- **Two proxies become the thing they were approximating, at no token cost.** "≤ 300 characters or it is probably a wontfix" and "a story about intent rather than a fact about the code" are the same length. One of them is checkable by the person answering.
