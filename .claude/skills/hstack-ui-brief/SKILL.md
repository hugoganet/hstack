...
- Verify the configured design system docs (paths from `hstack/config.yaml`) are reachable. Halt if absent — `ui-ux-briefer`'s contract refuses to brief against a missing design system.
