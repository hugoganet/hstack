...
description: |
  Use this skill when a tech-debt item is being closed without a fix because the team has decided the cost of resolving exceeds the cost of living with it. The Skill runs a brief two-question interview, captures the rationale into the TD's frontmatter, and flips status `open → wontfix` via `spec-author`. Wontfix is a terminal status — per TD-03, no field rewrites are permitted afterwards. Examples:
...
- Verify `hstack/tech-debt/<td-id>.md` exists and is at `status: open`. If at `in-progress`, halt — work is already underway; cancel the resolution change-spec first via `spec-author` then re-invoke. If at any terminal status, halt with the status named.
...
6. **Invoke `spec-author`.** Use the Task tool with `subagent_type: spec-author` and instructions: write `wontfix-reason: <answer-a>` and `wontfix-accepted-alternative: <answer-b>` to the TD's frontmatter, append a Resolution Log entry with date and owner, flip `status: open → wontfix`. Auto-commit at the status transition.
...
- **Spec-author write fails.** Halt; the TD's status flip and the two frontmatter writes must land in a single auto-commit. Partial writes are not possible if the kernel's auto-commit rule is honored.
...
- Never invoke `spec-author` to write the status flip in isolation from the rationale fields. The three writes (status, wontfix-reason, wontfix-accepted-alternative) land atomically.
