...
- `--interview <doc-name>` (optional): re-run the conversational interview for one of `vision`, `glossary`, `mvp-scope`, `personas`, `data-architecture`, `tech-stack`, `ci-cd`, `threat-model`, `hardening-checklist`, `incident-runbook`.
...
2. Invoke `product-manager` via Task with `subagent_type: product-manager` and context = [kernel, template, existing instance]. The subagent walks the document's fields, treating the existing values as the proposal layer and the engineer's responses as accept-or-correct.
