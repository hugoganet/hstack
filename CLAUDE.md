...
- **Subagent transcript resume.** Skills that may re-invoke the same subagent within one Claude Code session (notably `/hstack:implement` across plan phases) persist the returned `agentId` UUID to `hstack/.session-state/<skill>-<change-id>.yaml` on first spawn. Subsequent invocations attempt `SendMessage(to: <agent-uuid>, ...)` first — the harness resumes the agent from on-disk transcript with cache-read pricing on the prefix (measured ~12× cheaper than a fresh spawn for the same prefix; cache-create on the ~28k-token implementer prefix drops to ~60 tokens on resume). On any failure (different Claude Code session, transcript expired, agent unknown), the Skill falls back to `Agent(subagent_type: <type>, ...)` and overwrites the persisted UUID. Cross-Claude-Code-session resume is NOT load-bearing — the transcript path is keyed by Claude Code's session UUID and is gone once the session ends; fresh spawn is correct on session change. Single-shot subagents (`test-strategist`, `planner`, `security-reviewer`, `data-specialist`, `ui-ux-briefer`, `verifier`, `adversarial-reviewer`) skip this — one invocation per change, no resume to optimize. The resume payload must re-state the in-scope constraint and instruct the resumed agent to re-read `plan.md` and the change-spec, because between phases the on-disk state advanced (e.g., `steps-completed` got the previous phase appended) and the resumed agent's working memory is stale on those files. The `name:` Agent parameter is a separate, in-memory-only alias that lives only while the spawned process is alive; it is NOT useful for cross-phase resume and Skills should not rely on it.
